...
function redraw() {
  ...
  if (gd) {
    ...
    if (phi === 0) {
      ...
      let ptpi: number[] = Array(phi * phi);
      {
        // calculate inverse of (X^T X)
        ptpi.fill(0);
        for (let i = 0; i < ptpi.length; i += phi + 1) ptpi[i] = 1;
        let ptp = Array(phi * phi);
        for (let i = 0; i < phi; i++)
          for (let j = 0; j < phi; j++) ptp[i * phi + j] = sums[i + j];
      ...
      for (let i in xs) {
        ...
        for (let j = 0; j < phi; j++)
          for (let k = 0; k < phi; k++) tmp[j] += ptpi[j * phi + k] * phiX[k];
        let y = ys[i];
